...
describe('CelecService', () => {
  ...
  it('should correctly format date parts for Ecuador (GMT-5)', () => {
    ...
    const parts = service.getEcuadorDateParts(date);
    expect(parts).toEqual({ year: '2026', month: '06', day: '23', hora: 5 });
    
    const dateBoundary = new Date('2026-06-23T03:00:00.000Z');
    const partsBoundary = service.getEcuadorDateParts(dateBoundary);
    expect(partsBoundary).toEqual({ year: '2026', month: '06', day: '22', hora: 22 });
  });

  it('should fetch point values in raw CELEC array order', async () => {
    const mockData = {
      data: {
        items: [
          { loctimestamp: '2026-06-23T07:00:00Z', valueedit: '150.5' },
          { loctimestamp: '2026-06-23T06:00:00Z', valueedit: '148.2' }
        ]
      }
    };
  ...
  it('should fetch daily energy in raw CELEC array order for a specific date', async () => {
    const mockData = {
      data: {
        items: [
